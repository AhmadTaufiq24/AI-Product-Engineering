...
export const researchRouter = new Hono()
  .get('/', async (c) => {
    const researchs = await prisma.reseaech.findMany()
  ...
  .post('/',
    ...
    async (c) => {
      ...
      const newResearch = await prisma.reseaech.create({
        data: {
          jobTitle,
          level,
          industry,
      ...
      return c.json({ message: 'Research is on queue', researchId: newResearch.id })
    }
  );
