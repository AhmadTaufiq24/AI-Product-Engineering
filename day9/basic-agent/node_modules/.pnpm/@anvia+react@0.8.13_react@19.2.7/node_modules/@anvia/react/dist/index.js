...
function createUserMessage(input) {
  ...
  if (typeof input !== "string" && input.metadata !== void 0) {
...
function applyAnviaStreamEvent(messages, event) {
  ...
  if (event.type === "tool_result") {
    ...
    if (providerCallId !== void 0) part.callId = providerCallId;
    if (turn !== void 0) part.turn = turn;
...
function appendAssistantReasoningDelta(messages, delta, reasoningId) {
  ...
  if (reasoningId !== void 0) part.reasoningId = reasoningId;
...
function updateAssistantToolPart(messages, part, aliases = [], inputMatch) {
  ...
  if (existingPart !== void 0) {
    ...
    if (callId !== void 0) nextPart.callId = callId;
  }
  ...
  const sendMessages = useCallback(
    async (nextMessages, runOptions = {}) => {
      ...
      const createRequest = options.createRequest ?? ((args) => {
        ...
        if (args.resume !== void 0) request.resume = args.resume;
        return request;
      });
  ...
  const decideToolApproval = useCallback(
    async (approvalId, approved, reason) => {
      ...
      try {
        ...
        if (reason !== void 0) input.reason = reason;
        if (approval !== void 0) input.approval = approval;
  ...
  const answerToolQuestion = useCallback(
    async (questionId, answers) => {
      ...
      try {
        ...
        if (question !== void 0) input.question = question;
...
// src/use-smooth-stream-text.ts
import { useCallback as useCallback3, useEffect as useEffect3, useLayoutEffect, useRef as useRef3, useState as useState3 } from "react";
var DEFAULT_LARGE_APPEND_CHARS = 500;
var REDUCED_MOTION_QUERY = "(prefers-reduced-motion: reduce)";
var useIsomorphicLayoutEffect = typeof window === "undefined" ? useEffect3 : useLayoutEffect;
var smoothingConfigs = {
  realtime: { fraction: 0.5, maxChars: 24 },
  balanced: { fraction: 0.25, maxChars: 12 },
  silky: { fraction: 0.125, maxChars: 6 }
};
function useSmoothStreamText(content, options = {}) {
  ...
  const shouldAnimate = enabled && mode !== "none" && options.isStreaming !== false && !reducedMotion;
  const [text, setText] = useState3(content);
  const [isAnimating, setIsAnimating] = useState3(false);
  const targetRef = useRef3(content);
  const targetCharactersRef = useRef3([...content]);
  const displayedLengthRef = useRef3(targetCharactersRef.current.length);
  const frameRef = useRef3(void 0);
  const mountedRef = useRef3(true);
  const shouldAnimateRef = useRef3(shouldAnimate);
  const presetRef = useRef3(preset);
  shouldAnimateRef.current = shouldAnimate;
  presetRef.current = preset;
  const cancelFrame = useCallback3(() => {
    const frame = frameRef.current;
    frameRef.current = void 0;
    if (frame !== void 0 && typeof window !== "undefined" && typeof window.cancelAnimationFrame === "function") {
      window.cancelAnimationFrame(frame);
    }
  }, []);
  const syncToTarget = useCallback3(() => {
    cancelFrame();
    displayedLengthRef.current = targetCharactersRef.current.length;
    setText(targetRef.current);
    setIsAnimating(false);
  }, [cancelFrame]);
  const scheduleFrame = useCallback3(
    function scheduleFrame2() {
      if (frameRef.current !== void 0 || !mountedRef.current) {
        return;
      }
...
function useSystemReducedMotion(override, observe) {
  const [reducedMotion, setReducedMotion] = useState3(false);
  useEffect3(() => {
    if (!observe || override !== void 0 || typeof window === "undefined" || typeof window.matchMedia !== "function") {
      return;
    }
