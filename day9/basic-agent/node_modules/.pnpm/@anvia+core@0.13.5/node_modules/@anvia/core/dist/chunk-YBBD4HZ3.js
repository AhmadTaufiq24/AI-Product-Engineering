...
async function embedDocuments(model, documents, options) {
  ...
  return prepared.map((item, index) => {
    const embeddings2 = byDocument.get(index) ?? [];
    if (item.metadata === void 0) {
      return { id: item.id, document: item.document, embeddings: embeddings2 };
    }
    return { id: item.id, document: item.document, metadata: item.metadata, embeddings: embeddings2 };
  });
}
...
//# sourceMappingURL=chunk-YBBD4HZ3.js.map