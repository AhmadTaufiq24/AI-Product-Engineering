...
// src/openai/chat-completion.ts
var INVALID_TOOL_INDEX_ERROR = "OpenAI Chat Completions stream returned a tool call with an invalid index; expected a finite nonnegative integer.";
var AMBIGUOUS_CHOICE_ERROR = "OpenAI Chat Completions stream returned ambiguous completion choices without valid indices; provider output cannot be assembled safely.";
var MISSING_TOOL_FINISH_ERROR = "OpenAI Chat Completions tool-call stream ended without a terminal finish reason; provider output may be incomplete.";
var LENGTH_TOOL_FINISH_ERROR = 'OpenAI Chat Completions tool-call stream ended with finish_reason "length"; provider output may be incomplete.';
var CONTENT_FILTER_TOOL_FINISH_ERROR = 'OpenAI Chat Completions tool-call stream ended with finish_reason "content_filter"; tool calls will not be executed.';
var UNSUPPORTED_TOOL_FINISH_ERROR = "OpenAI Chat Completions tool-call stream ended with an unsupported finish reason; provider output cannot be assembled safely.";
var CONFLICTING_TOOL_FINISH_ERROR = "OpenAI Chat Completions tool-call stream returned conflicting terminal finish reasons; provider output cannot be assembled safely.";
...
function fromOpenAIChatCompletionResponse(response) {
  ...
  const firstChoice = choices.find((choice2) => isPlainObject(choice2) && choice2.index === 0) ?? choices.find(isPlainObject);
...
function mapOpenAIChatCompletionStreamChunk(chunk) {
  ...
  if (choice !== void 0 && isPlainObject(choice.delta)) {
  ...
  const mapping = {
    events,
    hasToolCalls,
    hasFinishReason: choice !== void 0 && isTerminalFinishReason(choice.finish_reason)
  };
...
var OpenAIChatCompletionToolStreamState = class {
  hasToolCalls = false;
  ...
  assertSupportedFinishReason() {
    ...
    if (this.finishReason !== "tool_calls" && this.finishReason !== "stop" && this.finishReason !== "function_call") {
      throw new Error(UNSUPPORTED_TOOL_FINISH_ERROR);
    }
  }
};
function primaryStreamChoice(value) {
  const choices = Array.isArray(value) ? value.filter(isPlainObject) : [];
  if (choices.length === 0) {
    return void 0;
  }
  const indexedChoices = [];
  const unindexedChoices = [];
  for (const choice of choices) {
    if (choice.index === void 0) {
...
function toolCallDelta(id, values) {
  ...
  if (values.callId !== void 0 && values.callId.length > 0) event.callId = values.callId;
  if (values.name !== void 0 && values.name.length > 0) event.name = values.name;
...
function embeddingDataFromResponse(response, inputLength) {
  const raw = response;
...
var OpenAIResponsesCompletionModel = class {
  ...
  traceRequest(request, options = {}) {
    ...
    return providerRequestSummary2(params, request, options);
  }
  async completion(request) {
...
function providerRequestSummary2(params, request, options) {
  return compactJsonObject2({
    ...
    model: stringFrom(params.model),
    parameterKeys: Object.keys(params).sort(),
    inputCount: Array.isArray(params.input) ? params.input.length : void 0,
    toolCount: request.tools.length,
    toolNames: request.tools.map((tool) => tool.name),
    ...
    temperature: request.temperature,
    maxTokens: request.maxTokens,
    toolChoice: toolChoiceSummary2(request.toolChoice),
    additionalParamKeys: isPlainObject(request.additionalParams) ? Object.keys(request.additionalParams).sort() : void 0
  });
}
function toolChoiceSummary2(toolChoice) {
  if (toolChoice === void 0 || typeof toolChoice === "string") {
    return toolChoice;
  }
  return { type: toolChoice.type, name: toolChoice.name };
...
function fromOpenAIResponse(response) {
  ...
  for (const item of output) {
    ...
    if (item.type === "function_call") {
      ...
      choice.push(AssistantContent2.toolCall(id, name, parseToolArguments(id, argsText), callId));
    }
...
function fromOpenAIStreamEvent(event) {
  ...
  if ((event.type === "response.completed" || event.type === "response.incomplete") && isPlainObject(event.response)) {
    return {
      type: "final",
...
function toolResultContentToOpenAIResponsesOutput(content) {
...
function messageOutputToAssistantContent(item) {
  ...
  return content.flatMap((part) => {
    ...
    if (part.type === "refusal" && typeof part.refusal === "string") {
      return [AssistantContent2.text(part.refusal)];
    }
    return [];
  });
}
...
async function collectModelsFromResponse(response) {
  ...
  if (isObject2(response) && Array.isArray(response.data)) {
    return response.data;
  }
  return [];
}
function toListedModel(model) {
  if (!isObject2(model) || typeof model.id !== "string") {
    return void 0;
  }
  const listedModel = {
    id: model.id
